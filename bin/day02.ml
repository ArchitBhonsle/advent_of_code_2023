...
let split_and_strip ~on x = x |> String.split ~on |> List.map ~f:String.strip
...
let parse_pull pull_str =
  let count_color = pull_str |> split_and_strip ~on:' ' |> list_to_pair in
  let count = count_color |> fst |> int_of_string in
  let default = { r = 0; g = 0; b = 0 } in
...
let parse_reveals reveals_str =
  reveals_str |> split_and_strip ~on:';' |> List.map ~f:parse_reveal
;;

let parse_game game_str =
  let split = game_str |> split_and_strip ~on:':' |> list_to_pair in
  (* let game_id_str = fst split  *)
  (* in *)
  let reveals_str = snd split in
