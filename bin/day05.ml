...
let parse_map_str map_str =
  ...
  |> List.map ~f:(fun l ->
    { dst = List.nth_exn l 0; src = List.nth_exn l 1; len = List.nth_exn l 2 })
;;
...
let minimum =
  seeds |> List.map ~f:(search maps) |> List.fold ~init:Int.max_value ~f:Int.min
;;
