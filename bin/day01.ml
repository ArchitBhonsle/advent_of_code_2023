...
let get_num possibilities found =
  match List.findi possibilities ~f:(fun _ p -> String.equal p found) with
  | Some idx -> fst idx + 1
...
let parse_line line =
  ...
  and backward =
    get_num words_rev (Re.Group.get (Re.exec backward_re (String.rev line)) 0)
  in
  (forward * 10) + backward
;;

let () = lines |> List.map ~f:parse_line |> List.fold ~init:0 ~f:( + ) |> printf "%d\n"
